...
provider "oci" {
  alias            = "home"
  fingerprint      = var.oci_admin_provider.api_fingerprint
  private_key_path = var.oci_admin_provider.api_private_key_path
  region           = var.oci_admin_general.home_region
  tenancy_ocid     = var.oci_admin_provider.tenancy_id
  user_ocid        = var.oci_admin_provider.user_id
}

data "oci_identity_compartments" "compartments_id" {
  access_level              = "ACCESSIBLE"
  compartment_id            = var.oci_admin_provider.tenancy_id
  compartment_id_in_subtree = "true"

  filter {
    name   = "id"
    values = [var.oci_admin_general.compartment_id]
  }
}
